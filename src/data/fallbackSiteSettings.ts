import type { SiteSettings } from "@/types/site";

export const FALLBACK_SITE_SETTINGS: SiteSettings = {
  brandName: "Mountain Rose",
  tagline:
    "Handmade genuine cow leather bags from Indonesia, made with a strong, elegant, and timeless character.",
  whatsappNumber: "6285223260530",
  additionalWhatsAppNumbers: ["6282124139283"],
  instagramUrl: "https://instagram.com/mountainrose.bag",
  email: "egarose46@gmail.com",
  additionalEmails: ["dini.andini2011@gmail.com"],
  address: "Lagoon Avenue Mall GF 54 Bekasi",
  logoUrl: "/mountain-rose-logo-transparent.png",
};
